...
const ACTION_SEPERATION_TIME = 20; // time in ms between actions
...
const HOME_ALLOCATED_RAM = 131000;
// GUI //
const BASE_BACKGROUND_COLOR = "#000000";
const CONTENT_BACKGROUND_COLOR = "#1A1C24";
const TRINARY_BACKGROUND_COLOR = "#25293b";

const TASK_GUI_WIDTH = 90;
...
let bitnode = undefined;
/** @param {NS} NETSCANAPI */
export async function main(NETSCANAPI) {
    ns = NETSCANAPI;
    ns.disableLog("ALL");
    ns.tail();
...
// min avg max
let TASK_BATCH_TIME = [Infinity, 0, -Infinity];
let BATCH_TIME = [Infinity, 0, -Infinity];
/** @param {BotServer} target */
async function batch(target) {
    ...
    if (tasks.length > 0) {
        ...
        tasks.forEach(elm=> {target.timeOffset = Math.max(elm[3] + startTime, target.timeOffset)});
        let maxTime = 0;
...
function blog(...log) {
    //ns.tprint("[BATCHER] " + log.join(" "));
}

let player;
/** @param {BotServer} _target */
    ...
    while (true) {
        ...
        if (state == STATES.GROW || state == STATES.PREP) {
            ...
            if (growExecutions == 0) {
                ...
                //ns.tprint(`[BATCH PREDICTION] Simulation prediction: ${target_server.moneyAvailable}`)
                //ns.tprint(`[BATCH PREDICTION] Simulation multiplyer prediction: ${(preGrowAmount + growExecutions) * growMultiplyer}`)
...
function getGrowLog(server, threads, cores) {
    ...
    let bitnodeMult;
    try { bitnodeMult = ns.getBitNodeMultipliers().ServerGrowthRate }
    catch { bitnodeMult = 1 }
...
function getGrowAmount(server, threads, cores) {
    ...
    if (serverGrowth < 1) { serverGrowth = 1; }
...
let MAX_RAM = 0;
let GLOBAL_OFFSET = 0;
let INCREASE_GLOBAL_OFFSET = false;
class BotServer {
    constructor(server) {
        /** @type {Server} */
        this.server = server;
    ...
    start(task) {
        ...
        let _a = task._arguments !== undefined ? task._arguments : [];
        let p = 0;
        try {
    ...
    get backdoorInstalled() { return this.refresh("backdoorInstalled"); }
    get cpuCores() { return this.refresh("cpuCores"); }
    get hackDifficulty() { return this.refresh("hackDifficulty"); }
    get hasAdminRights() { return this.refresh("hasAdminRights"); }
    get isConnectedTo() { return this.refresh("isConnectedTo"); }
    get moneyAvailable() { return this.refresh("moneyAvailable"); }
    get openPortCount() { return this.refresh("openPortCount"); }
    get ramUsed() { return this.refresh("ramUsed"); }
    get smtpPortOpen() { return this.refresh("smtpPortOpen"); }
    get sqlPortOpen() { return this.refresh("sqlPortOpen"); }
    get sshPortOpen() { return this.refresh("sshPortOpen"); }

    get ftpPortOpen() { return this.refresh("ftpPortOpen"); }
    get httpPortOpen() { return this.refresh("httpPortOpen"); }
    ...
    get holdRam() {return this._holdRam};
    get holdRam2() {return this._holdRam2};
...
function updateServerList() {
    ...
    */
    ...
    let changed = false;
    for (let elm of s) {
        if (isAdmin(elm) && !SERVER_LIST.map(elm => elm.hostname).includes(elm.hostname)) {
...
///////// REACT
const React = _window.React;
const ReactDOM = _window.ReactDOM;
const Elm = React.createElement;
let guiState, setGuiStateRaw;
...
const StrawberryCoreGUIComponent = () => {
...
const createSimulationElement = () => {
    ...
    return Elm("div", {
        ...
        Elm("div", {
            ...
            `Batch state: ${BATCH_STATE}`, Elm("br"),
            `Batch exit: ${BATCH_EXIT}`, Elm("br"),
            ...
            Elm("br"),
            Elm("br"),
            `Security Level: ${sim.hackDifficulty}/${sim.minDifficulty} (${Math.round((sim.hackDifficulty/sim.minDifficulty) * 1000)/10}%)`
        ),
        Elm("div", {
                style: {
                    "top": "-4px",
