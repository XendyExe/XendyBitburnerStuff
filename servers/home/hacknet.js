...
/** @param {NS} ns */
function calculateBestBuyNode(ns, index) {
  ...
  let ramWorth = ramUpgradeProd/ns.hacknet.getRamUpgradeCost(index);
  let levelWorth = levelUpgradeProd/ns.hacknet.getLevelUpgradeCost(index);
  let coreWorth = coreUpgradeProd/ns.hacknet.getCoreUpgradeCost(index);
...
async function waitForMoney(m) {
  while (ns.getPlayer().money < m && await ns.sleep(1000)) { if (ns.getPlayer().money > m) break; }
}
/** @type {NS} */
let ns;
/** @param {NS} ns */
