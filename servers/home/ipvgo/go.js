...
async function findGoodGame() {
    ...
    while (await ns.sleep(1)) {
        ...
        if (state[2][0] == "#") { rotation = 0; }
        else if (state[4][2] == "#") { rotation = 1; }
        else if (state[2][4] == "#") { rotation = 2; }
        else if (state[0][2] == "#") { rotation = 3; }
        else continue;
        break;
    }
    let reflection = (rotation == 0 && initialCords[0] == 3) ||
        (rotation == 1 && initialCords[1] == 3) ||
        (rotation == 2 && initialCords[0] == 1) ||
        (rotation == 3 && initialCords[1] == 1);
...
function rotate(x, y, clockwiseAmount) {
    const n = 5; // Grid size (5x5)
    let rotatedX, rotatedY;

    // Normalize the clockwiseAmount to be between 0 and 3
    clockwiseAmount = ((clockwiseAmount % 4) + 4) % 4;

    switch (clockwiseAmount) {
        case 0:
            // 0 degrees rotation (original coordinates)
            rotatedX = x;
            rotatedY = y;
            break;
        case 1:
        ...
        case 2:
            ...
            break;
        case 3:
            // 270 degrees clockwise rotation (or 90 degrees counterclockwise)
            rotatedX = y;
            rotatedY = n - 1 - x;
            break;
    }
...
function rotateArray(array, clockwiseAmount) {
    // Helper function to rotate the matrix clockwise by 90 degrees
    ...
    // Parse the array of strings into a 2D array of characters
    ...
    // Rotate the matrix the required number of times
    for (let i = 0; i < rotations; i++) {
        matrix = rotate90Clockwise(matrix);
    }

    // Convert the 2D array back into an array of strings
...
function mirrorArray(array) {
    ...
    // Reverse each row (mirror across Y-axis)
    let mirroredMatrix = matrix.map(row => row.reverse());

    // Convert the mirrored 2D array back into an array of strings
...
export async function main(_ns) {
    ...
    while (true) {
        ...
        const tryMove = async (x, y) => {
            ...
            // log("Actual move on ", pos, "after", rotation-4, "rotations and reflection:", reflection)
        ...
        while (await ns.sleep(1)) {
            ...
            if (gameMap[gameState] !== undefined) {
